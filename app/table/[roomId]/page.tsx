...
export default function TablePage() {
  ...
  const [currency, setCurrency] = useState<string>("chips"); // fetched from server
  ...
  useEffect(() => {
    ...
    // Connect briefly to fetch room currency
  ...
  const showBuyIn = !isChips; // only show buy-in for real money games
  ...
  const handleJoin = () => {
    ...
    if (typeof sessionStorage !== "undefined") {
      ...
      if (buyIn && showBuyIn) sessionStorage.setItem("join_room_buyin", buyIn);
    }
    setJoining(true);
    ...
    <div style={{ minHeight:"100dvh", background:"#0a0a0f", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"system-ui,sans-serif", padding:16 }}>
      <div style={{ width:"100%", maxWidth:400, padding:32, background:"rgba(15,23,42,0.95)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, boxShadow:"0 20px 60px rgba(0,0,0,0.8)" }}>
        ...
        <button onClick={handleJoin} disabled={joining || loadingRoom}
          style={{ width:"100%", padding:"15px 0", background: (joining||loadingRoom) ? "rgba(67,56,202,0.5)" : "#4338ca", border:"none", borderRadius:13, color:"#fff", fontSize:16, fontWeight:700, cursor: joining ? "wait" : "pointer" }}>
          {loadingRoom ? "Loading room…" : joining ? "Joining…" : "JOIN TABLE →"}
        </button>

        <p style={{ textAlign:"center", color:"#334155", fontSize:10, marginTop:14 }}>Link expires if the host closes their tab — ask for a new one</p>
      </div>
    </div>
  );
}
